...
jQuery.fn.insertRoundCaret = function(strStart, strEnd, strReplace) {
   return this.each(function() {
      if (document.selection) {
         ...
         this.focus();
         sel = document.selection.createRange();
         insertString = strReplace ? strReplace : sel.text.trim();
         ...
         var precedingSpaces = strReplace.match(/^\s+/);
         precedingSpaces = (precedingSpaces != null) ? precedingSpaces[0] : '';
         var endingSpaces = strReplace.match(/\s+$/);
         endingSpaces = (endingSpaces != null) ? endingSpaces[0] : '';
         strReplace = strReplace.trim();

         this.value = this.value.substring(0, startPos) + precedingSpaces + strStart
                    + strReplace + strEnd + endingSpaces
                    + this.value.substring(endPos, this.value.length);
         this.focus();
         this.selectionStart = startPos + strStart.length + precedingSpaces.length;
